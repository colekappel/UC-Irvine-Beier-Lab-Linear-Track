...
if  mouseNum == "6412" || mouseNum == "6416" || mouseNum == "6431"
...
elseif PLUSORMINUS==-1
    FadPlusorMinus = ones(length(avAmplitudeForEachCell),1)*-1; %1 for plus, 0 for minus
end
...
while i< length(allXVals)
    ...
    if allXVals(i)>=700 && allXVals(i)<=950
        ...
        while allXVals(i) <=endXCoord && allXVals(i) >=200
        ...
        while allXVals(index) >=200 && allXVals(index) <=endXCoord
            if index==1
                break
            end
            index=index-1;
...
for i = 1:height(eventRatePerBinMatx)
   stability1v2(i)=atan(corr(transpose(eventRatePerBinMatx1(i,1:width(eventRatePerBinMatx))),transpose(eventRatePerBinMatx2(i,1:width(eventRatePerBinMatx))),'type','Pearson'));
...
for i = 1:height(eventRatePerBinMatx)
   stabilityOddEven(i)=atan(corr(transpose(eventRatePerBinMatx11(i,1:width(eventRatePerBinMatx))),transpose(eventRatePerBinMatx22(i,1:width(eventRatePerBinMatx))),'type','Pearson'));
...
save(filename,"stabilityAverage","stability1v2","stabilityOddEven",...
    ...
    "spikesWXVal","placeCells","averageSpeedPerCrossing",...
    "distanceTraveledPerCrossing","speedPerCrossing")
end