function[] = AnalysisFunctionPlaceCells(spikes,ezTrack,FR,inscopixStartTimeInSeconds,ezTrackStart,...
    ezTrackEnd,mouseNum,PLUSORMINUS,NUMBEROFBINS,cohortID)
% use 1 for plus, -1 for minus ie:
% vvv these need to be edited for each mouse vvv 
%{
spikes = readtable('Data to Analyze/5507.spikes.LT-imaging-T1.csv');
ezTrack = readtable('Data to Analyze/WIN_20230601_5507_LT_Imaging_T1_LocationOutput.csv');
FR=14.255;
inscopixStartTimeInSeconds = 172/FR; %frames over frame rate
ezTrackStart=470/FR; %frames over frame rate
ezTrackEnd=13713/FR; %frames over frame rate
mouseNum = "5507";
PLUSORMINUS=1; % 1 for plus 0 for minus
cohortID=0;
%}
% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

ezTrack{:,11}=ezTrack{:,7}/FR; % add time in seconds to table in 12th column
ezTrack.Properties.VariableNames([11])="Time in Seconds";
...
allTimesEzTrack = table2array(ezTrack(:,11));
...
%% code to get rid of bins with low activity:
%
rowsToDelete=[];
index=1;
for j = 1: height(eventPerBinMatx)
    if max(eventRatePerBinMatx(j,:))<0.1
        rowsToDelete(index)=j;
        index=index+1;
    end
end
eventRatePerBinMatx(rowsToDelete,:)=[];
eventPerBinMatx(rowsToDelete,:)=[];
%}
...
sparsity = sparsityFunc(eventRatePerBinMatx,Pi);
...
for p = 1:numCells
     ...
     while ismember(cellNamesNew(p,1),spikesWXVal(i,2))
        ...
        amplitudeArray(z)=spikesWXVal{i,3};
        i=i+1;
        z=z+1;
        a=a+1;
...
for i = 1:length(theInfoScore)
    if theInfoScore(i)>prctile(infoScoreMatx(i,:),95)
        ...
        erPlaceCells(b)=eventRatePerCell(b);
        ampPlaceCells(b)=avAmplitudeForEachCell(b);
...
%now code to visualize diagonal line: start w 5507
if isempty(infoScoresplaceCells)
    sortedPlaceCells=[];
else
orderMatx=[];
...
% For Amplitude:
ampPlaceCells=transpose(ampPlaceCells);

if PLUSORMINUS==1
    FadPlusorMinus = ones(length(ampPlaceCells),1); %1 for plus, 0 for minus
elseif PLUSORMINUS==-1
    FadPlusorMinus = ones(length(ampPlaceCells),1)*-1; %1 for plus, 0 for minus
end
MouseID=strings(length(ampPlaceCells),1);
for i = 1:length(ampPlaceCells)
    MouseID(i)=mouseNum;
end
avAmplitudeForEachCell=[];
avAmplitudeForEachCell = ampPlaceCells;
LMETableAmplitude=table(avAmplitudeForEachCell,FadPlusorMinus,MouseID);

%for event rate:
erPlaceCells=transpose(erPlaceCells);
eventRatePerCell =[];
eventRatePerCell = erPlaceCells;
LMETableEventRate=table(eventRatePerCell,FadPlusorMinus,MouseID);

%for info score

infoScorePlaceCells = infoScore(placeCellsArr,Pi);
theInfoScore = [];
theInfoScore=infoScorePlaceCells;
LMETableInfoScore=table(theInfoScore,FadPlusorMinus,MouseID);

%for sparsity

sparsityPlaceCells = sparsityFunc(placeCellsArr,Pi);
sparsity =[];
sparsity=transpose(sparsityPlaceCells);
...
[rrPC,cols]=find(ismember(spikesWXVal{:,2}, cellstr(placeCells) ));

spikesWXValPlaceCells= spikesWXVal(rrPC,:);
leftOrRight_PlaceCells= spikesWXValPlaceCells{:,5};
placeCells = cellstr(transpose(placeCells));
...
[RowsSpikes1,ColsSpikes1]=find(spikesWXValPlaceCells{:,1}<=(max(allTimesEzTrack)/2));
[RowsSpikes2,ColsSpikes2]=find(spikesWXValPlaceCells{:,1}>(max(allTimesEzTrack)/2));
spikesWXVal1 =spikesWXValPlaceCells(RowsSpikes1,:);
spikesWXVal2 =spikesWXValPlaceCells(RowsSpikes2,:);
leftOrRight_spikes1=leftOrRight_PlaceCells(RowsSpikes1,:);
leftOrRight_spikes2=leftOrRight_PlaceCells(RowsSpikes2,:);
...
[Pi1,mouseSecsInBinX1,eventPerBinMatx1,eventRatePerBinMatx1]...
    = getEventRatePerBinMatx(NUMBEROFBINS,BINSIZE,allXValsWDirection1,placeCells,spikesWXVal1,leftOrRight_spikes1,FR);
%2nd half:
[Pi2,mouseSecsInBinX2,eventPerBinMatx2,eventRatePerBinMatx2]...
    = getEventRatePerBinMatx(NUMBEROFBINS,BINSIZE,allXValsWDirection2,placeCells,spikesWXVal2,leftOrRight_spikes2,FR);

%% code to get rid of bins with low activity for stability: delete all cells
% in both eventperbinmatx with activity less than threshold
%{
rowsToDelete=[];
index=1;
for j = 1: height(eventPerBinMatx1)
    if max(eventRatePerBinMatx1(j,:))<0.1
        rowsToDelete(index)=j;
        index=index+1;
    end
end

for j = 1: height(eventPerBinMatx2)
    if max(eventRatePerBinMatx2(j,:))<0.1
        rowsToDelete(index)=j;
        index=index+1;
    end
end
...
eventRatePerBinMatx1(rowsToDelete,:)=[];
eventPerBinMatx1(rowsToDelete,:)=[];
eventRatePerBinMatx2(rowsToDelete,:)=[];
eventPerBinMatx2(rowsToDelete,:)=[];
%}
%% continue stability calculations

stability1v2 = zeros(1,height(eventRatePerBinMatx1));
for i = 1:height(eventRatePerBinMatx1)
   stability1v2(i)=atan(corr(transpose(eventRatePerBinMatx1(i,1:width(eventRatePerBinMatx1))),transpose(eventRatePerBinMatx2(i,1:width(eventRatePerBinMatx1))),'type','Pearson'));
...
while ii < (length(timePerCrossing_Time)-3)
    [rowsSpikes11,colsSpikes11]=...
        find(spikesWXValPlaceCells{:,1}>=timePerCrossing_Time(ii) & spikesWXValPlaceCells{:,1}<=timePerCrossing_Time(ii+3));
    [rowsAllX11,colsAllX11]=...
        find(allTimesEzTrack(:,1)>=timePerCrossing_Time(ii) & allTimesEzTrack(:,1)<=timePerCrossing_Time(ii+3));
    spikesWXVal11=[spikesWXVal11;spikesWXValPlaceCells(rowsSpikes11,:)];
    leftOrRight_spikes11=[leftOrRight_spikes11;leftOrRight_PlaceCells(rowsSpikes11,:)];
...
while ii < (length(timePerCrossing_Time)-3)
    ...
    [rowsAllX22,colsAllX22]=...
        find(allTimesEzTrack(:,1)>=timePerCrossing_Time(ii) & allTimesEzTrack(:,1)<=timePerCrossing_Time(ii+3));
    spikesWXVal22=[spikesWXVal22;spikesWXValPlaceCells(rowsSpikes22,:)];
    leftOrRight_spikes22=[leftOrRight_spikes22;leftOrRight_PlaceCells(rowsSpikes22,:)];
...
[Pi11,mouseSecsInBinX11,eventPerBinMatx11,eventRatePerBinMatx11]...
    = getEventRatePerBinMatx(NUMBEROFBINS,BINSIZE,allXValsWDirection11,placeCells,spikesWXVal11,leftOrRight_spikes11,FR);
%even:
[Pi22,mouseSecsInBinX22,eventPerBinMatx22,eventRatePerBinMatx22]...
    = getEventRatePerBinMatx(NUMBEROFBINS,BINSIZE,allXValsWDirection22,placeCells,spikesWXVal22,leftOrRight_spikes22,FR);
...
rowsToDelete=[];
index=1;
for j = 1: height(eventPerBinMatx11)
    if max(eventRatePerBinMatx11(j,:))<0.1
        rowsToDelete(index)=j;
        index=index+1;
    end
end

for j = 1: height(eventPerBinMatx22)
    if max(eventRatePerBinMatx22(j,:))<0.1
        rowsToDelete(index)=j;
        index=index+1;
    end
end
...
eventRatePerBinMatx11(rowsToDelete,:)=[];
eventPerBinMatx11(rowsToDelete,:)=[];
eventRatePerBinMatx22(rowsToDelete,:)=[];
eventPerBinMatx22(rowsToDelete,:)=[];
%}

%% continue stability calculations
stabilityOddEven = zeros(1,height(eventRatePerBinMatx11));
for i = 1:height(eventRatePerBinMatx11)
   stabilityOddEven(i)=atan(corr(transpose(eventRatePerBinMatx11(i,1:width(eventRatePerBinMatx11))),transpose(eventRatePerBinMatx22(i,1:width(eventRatePerBinMatx11))),'type','Pearson'));
...
if length(stabilityOddEven)<length(stability1v2)
    gg= length(stabilityOddEven);
else
    gg= length(stability1v2);
end
...
FadPlusorMinus = FadPlusorMinus(1:length(stability1v2));
MouseID = MouseID(1:length(stability1v2));
...
if PLUSORMINUS==1
    FadPlusorMinus = ones(length(stabilityOddEven),1); %1 for plus, 0 for minus
elseif PLUSORMINUS==-1
    FadPlusorMinus = ones(length(stabilityOddEven),1)*-1; %1 for plus, 0 for minus
end
MouseID=strings(length(stabilityOddEven),1);
...
if PLUSORMINUS==1
    FadPlusorMinus = ones(length(stabilityAverage),1); %1 for plus, 0 for minus
elseif PLUSORMINUS==-1
    FadPlusorMinus = ones(length(stabilityAverage),1)*-1; %1 for plus, 0 for minus
end
MouseID=strings(length(stabilityAverage),1);
for i = 1:length(stabilityAverage)
    MouseID(i)=mouseNum;
end
...
disp(strcat("Mouse Data/Data_",mouseNum,"EndBinsRemoved_RanSuccessfully"));
filename = strcat("Mouse Data/Data_",mouseNum,"EndBinsRemoved");

%% signal to noise ratio (snr) calculation
%% have to normalize the table first --- calculation is wrong rn
normedPCArr =zeros(size(placeCellsArr));
for i = 1: height(placeCellsArr)
    normedPCArr(i,:)=placeCellsArr(i,:)./max(placeCellsArr(i,:));
end

snr=zeros(height(normedPCArr),1);
for i = 1:height(normedPCArr)
    snr(i,1)=sum(normedPCArr(i,:))-1;
end

%% lme table for snr: 
FadPlusorMinus=[];
if PLUSORMINUS==1
    FadPlusorMinus = ones(length(snr),1); %1 for plus, 0 for minus
elseif PLUSORMINUS==-1
    FadPlusorMinus = ones(length(snr),1)*-1; %1 for plus, 0 for minus
end
MouseID=strings(length(snr),1);
for i = 1:length(snr)
    MouseID(i,1)=mouseNum;
end

LMETableSNR =table(snr,FadPlusorMinus, MouseID);

save(filename,"stabilityAverage","stability1v2","stabilityOddEven",...
    "theInfoScore","sparsity","avAmplitudeForEachCell",...
    ...
    "LMETableSparsity","placeCellsArr","infoScorePlaceCells",...
    "erPlaceCells","ampPlaceCells","sparsityPlaceCells", "snr","LMETableSNR","normedPCArr")
end